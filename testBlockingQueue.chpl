...
/*class Job {
    ...
    var timeToProcess : real;
    var timeCreated : real = getCurrentTime();
    var timeCompleted : real;
    
    proc execute() : real {
        sleep(timeToProcess);
        timeCompleted = getCurrentTime();
        var timeAlive = timeCompleted - timeCreated;
        writeln("Job executed!  Delay factor: " + (timeAlive / timeToProcess));
        return timeAlive;
    }
    
    proc string() : string {
        return timeToProcess : string;
    }
...
//to never stop, set secondsToPerish to negative
proc produce(queue : BlockingQueue(Job), gen : NPBRandomStream, maxSleepTime, maxJobLength, secondsToPerish) {
    var perishTime = getCurrentTime() + secondsToPerish;
    
    while (secondsToPerish < 0 || getCurrentTime() < perishTime) {
        var job = new Job(gen.getNext() * maxJobLength); //create the new job
        queue.add(job);
        writeln("Job added.    Queue: \n" + queue.toString());
        var sleepTime = (gen.getNext() * maxSleepTime);
        sleep(sleepTime);
    }
    
    writeln("The Producer is done!!!!!!!!!!!!!!!!!!!!!!");
    
}

proc consume(queue : BlockingQueue(Job), gen: NPBRandomStream, secondsToPerish) {
    var perishTime = getCurrentTime() + secondsToPerish;
    
    while (secondsToPerish < 0 || getCurrentTime() < perishTime || queue.getNumElements() > 0) {
        var job = queue.remove();
        writeln("Executing a job...  Queue: \n" + queue.toString());
        job.execute();
    }
    
    writeln("The Consumer is done!");
    
}

//run a test on a new queue
proc testQueue(queueSize : int, gen : NPBRandomStream, maxTimeBetweenAdds, consumerDelay, maxJobLength, secondsAlive, numProducers, numConsumers) {

    var queue = new BlockingQueue(Job, queueSize);
    
    sync{

        begin {
            forall i in 1..numProducers { 
                produce(queue, gen, maxTimeBetweenAdds * numProducers, maxJobLength * numConsumers, secondsAlive); 
            }
        }

        sleep(consumerDelay); //wait
        begin {
            forall i in 1..numConsumers {
                consume(queue, gen, secondsAlive - consumerDelay - 1);
            } 
        }
        writeln("Consumers launched!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
    }
...
//testQueue(capacity, generator, maxTimeBetweenAdds, consumerDelay, maxJobLength, secondsAlive, numProducers, numConsumers);
writeln("This first test produces and consumes at about the same rate.  (Press enter to run.)");
stdin.readln();

testQueue(18, rng, 2, 7, 2, 30, 1, 1);

writeln("The next test produces faster than it consumes.  (Press enter to run.)");
stdin.readln();

testQueue(18, rng, 1, 4, 4, 20, 1, 1);

writeln("This next test uses multiple producers and consumers.  (Press enter to run.)");
stdin.readln();

testQueue(5, rng, 2, 1, 3, 60, 4, 4);

writeln("This next test consumes faster than it produces.  (Press enter to run.)");
stdin.readln();

testQueue(18, rng, 2, 10, 1, 30, 1, 1);

writeln("All tests have completed.");

*/

/*
writeln("That test should have had no two threads running simultaneously.");
writeln("Press enter when ready.");
stdin.readln();

testSemaphore(15, 3, 5);

writeln("Press enter when ready.");
stdin.readln();
writeln("Nice!  On to the next test...");

testSemaphore(200, 20, 1);

writeln("Press enter when ready.");
stdin.readln();
writeln("Nice!  On to the next test...");

writeln("This test should fail to do anything.  Press Ctrl + C when you're confident that the students' code is correctly deadlocked.");

testSemaphore(10, 0, 1);
 */
